...
exports.rankScores = rankScores;
exports.missingFromRankings = missingFromRankings;
exports.mergeLateScores = mergeLateScores;
/**
...
function rankScores(scores) {
    ...
    return ordenados.map((s, i) => {
        if (i > 0 && s.score < ordenados[i - 1].score)
            lugar = i + 1;
        return { ...s, rank: lugar };
    });
}
 ...
 */
function missingFromRankings(existing, scores) {
    const yaEstan = new Set(existing.map((r) => r.playerId));
    const vistos = new Set();
    return scores.filter((s) => {
        if (yaEstan.has(s.playerId) || vistos.has(s.playerId))
            return false;
 ...
 */
function mergeLateScores(existing, late, isRanked) {
    if (late.length === 0)
        return existing;
    const nuevas = late.map((s) => ({
        playerId: s.playerId,
...
//# sourceMappingURL=lateRankings.js.map